...
새 traceroute producer는 실패 집계를 목적지 미도달과 실행 실패로 분리한다. `attempts_unreached`, `attempts_execution_failed`, `attempts_timed_out`, `attempts_cancelled`가 각각 기록되며, 혼합 실행 오류의 top-level code는 순서와 무관한 `traceroute_execution_incomplete`다. GeoIP enrichment 실패가 있으면 `geoip_provider_failures`가 기록되고 분석 coverage에 반영된다.
