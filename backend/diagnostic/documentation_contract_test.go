...
func TestDocumentationContractMatchesCurrentSourceAndFixtures(t *testing.T) {
	...
	currentDocClaims := []string{
		"Stage 9", "Web 298", "Compose SemVer", "default 2D/optional 3D", "기본 2D", "선택 가능한 3D",
	...
	requiredByDoc := map[string][]string{
		"README.md": {
			...
			"`MAX_TARGETS=20`", "`MAX_DOCUMENT_ELEMENTS=1200`", "Compose SemVer regression fix 뒤 현재 Stage 9 Web source inventory는 298 tests", "result shapes 31", "expanded result matrix 136", "eight producer traceroute witness fixtures", "runner-owned `cancelled`", "publication 전에 report 전체를 거부", "exact real release-gate fake cases 12",
		...
		"SPEC.md": {
			...
			"`MAX_TARGETS=20`", "`MAX_DOCUMENT_ELEMENTS=1200`", "Compose SemVer regression fix 뒤 현재 Web inventory는 298 tests", "31 result shapes", "136-row", "exact eight producer traceroute witness fixtures", "full raw", "Geo aggregate", "optional representative", "runner-owned `cancelled`", "whole report를 atomic 거부", "fake cases 12",
		...
		"docs/ARCHITECTURE.md": {
			...
			"umask 077/027/000 세 pass", "canonical Docker inventory equality", "Compose SemVer fix 뒤 현재 Web source inventory는 298 tests", "ignored `checknetwork-api` binary",
		},
		"docs/API.md": {
		...
		"docs/OPERATIONS.md": {
			...
			"Stage 8 historical source inventory는 Web 260 tests", "Compose SemVer fix 뒤 current source inventory는 Web 298 tests", "direct-child XML 297 tests + variant canaries 2", "result shapes 31", "result matrix 136", "ignored `checknetwork-api` binary", "비파괴 요청 때문에 제거하지 않고 retained",
		},
		"docs/TESTING.md": {
		...
		"docs/PLAN.md": {
			...
			"`MAX_TARGETS=20`", "`MAX_DOCUMENT_ELEMENTS=1200`", "Historical Stage 9 plan/closure record: current source inventory was Web 283 tests", "Compose SemVer fix 뒤 current source-level Web inventory 298", "result shapes 31", "expanded matrix 136", "456 semantic mutations", "13 generic cancelled-detail rejections", "eight traceroute witness fixtures", "raw-only", "compact/Geo aggregate", "optional", "generic cancelled", "fake cases 12", "ignored root `checknetwork-api` binary",
