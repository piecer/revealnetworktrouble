...
현재 stable source inventory는 producer findings 23, result shapes 31과 expanded result matrix 136, presentation `6/10/21/23/33`, API errors 16와 structural mutations 85, Web 260 tests(456 fixture-derived semantic mutations, 그중 generic cancelled-detail 13), Android debug/release 각각 direct-child XML 297 tests + variant canaries 2, API/Web offline archive validator 28+25=53 tests, exact real release-gate fake cases 12다. Production exact gate는 canonical inventory를 기준으로 umask 077/027/000 세 pass를 비교한다. 최신 parent focused gates와 Task 1~15 task-level reviews는 blocker 0 / major 0이다. 이 문서 edit 뒤의 final clean-environment `make ci`, 새 external manifest, manifest-derived temporary release, five-way review, Stage 8 commit과 exact-SHA closure는 아직 pending이다. Repository root의 ignored `checknetwork-api` binary는 candidate manifest와 검증 증거에서 제외되어 있으나 비파괴 요청 때문에 제거하지 않고 retained 상태다. 이를 final artifact-cleanup 또는 release 성공으로 해석하지 않는다.
