...
if [ -f "$source_dir/frontend/Dockerfile" ]; then
    ...
    docker exec "$web_container_id" sh -c 'for f in app.js index.html state.js styles.css topology-model.js topology-renderer.js topology-presentation.js topology-visualizer.js; do test -f "/usr/share/nginx/html/$f" || exit 1; done'
