...
if [ -f "$source_dir/frontend/Dockerfile" ]; then
    ...
    # shellcheck source=release-resource-ownership.sh
    . "$source_dir/scripts/release-resource-ownership.sh"
    for asset in nginx.conf app.js index.html state.js styles.css topology-model.js topology-renderer.js geo-map.js topology-presentation.js topology-visualizer.js; do
    ...
    docker exec "$web_container_id" sh -c 'for f in app.js index.html state.js styles.css topology-model.js topology-renderer.js geo-map.js topology-presentation.js topology-visualizer.js; do test -f "/usr/share/nginx/html/$f" || exit 1; done'
